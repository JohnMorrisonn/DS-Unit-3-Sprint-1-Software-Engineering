...
When reviewing code the most important part to me is PEP8 format, particularly spaces.
The most overwhelming thing to see is everything clumped together and makes it
dramatically more difficult to interpret. Looking at a function with no spaces between
for loops, list creations, creating variables, and different outputs can be really
stressful.

A container is useful because it allows for multiple workloads to run on ONE OS compared
to a VM that runs multiple instances of OS. The container would help to pack multiple
applications into one server and use much less CPU than a VM.
